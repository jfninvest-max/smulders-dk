    ...
    <main className="pt-20">
      <section className="relative h-[55vh] overflow-hidden flex items-end">
        <Image
          ...
          alt="Smulders staldboksdøre — tre modeller uden skarpe kanter"
          fill
          priority
        ...
        <div className="relative z-10 px-6 pb-16 max-w-7xl mx-auto w-full">
          <motion.div initial="hidden" animate="visible" variants={stagger}>
            ...
            <motion.h1 variants={fade} className="font-display text-4xl md:text-6xl font-normal mb-4">Staldboksdøre</motion.h1>
        ...
        <div className="grid md:grid-cols-2 gap-16 mb-20">
            ...
            <motion.div variants={fade} className="flex flex-col gap-4 mb-8">
              {models.map((m, i) => (
                <div key={i} className="flex gap-4 items-center border border-white/8 overflow-hidden">
                  <Image
                    src={m.img}
                    alt={`${m.name} — Smulders staldboksdør`}
                    width={96}
                    height={80}
          ...
          <motion.div initial={{ opacity: 1, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7 }} className="relative aspect-[4/3] overflow-hidden">
            <Image
              ...
              alt="Smulders staldboksdør med rustfrit stål greb og låsemekanisme"
              fill
              loading="lazy"
              className="object-cover"
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {images.map((src, i) => (
            <motion.div key={i} initial={{ opacity: 1, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, delay: i * 0.04 }} className="relative aspect-[4/3] overflow-hidden">
              <Image
                src={src}
                alt={`Staldboksdør galleri — billede ${i + 1}`}
                fill
                loading="lazy"
      ...
      <section className="py-10 md:py-20 px-6 text-center border-t border-white/5">
        <motion.div initial="hidden" animate="visible" variants={stagger}>
          <motion.h2 variants={fade} className="font-display text-3xl font-normal mb-5">Bestil dine staldboksdøre</motion.h2>
